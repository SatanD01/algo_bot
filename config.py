...
MIN_STOPLOSS_PIPS = 20  # Минимальный стоп-лосс в пипсах (увеличен для большей надежности)
MAX_POSITIONS = 1  # Максимальное количество одновременно открытых позиций
MAX_DAILY_RISK = 0.01  # Максимальный дневной риск (1% от депозита)
...
TRAILING_ACTIVATION = 0.5  # Активация трейлинга при достижении 50% от стоп-лосса
BREAKEVEN_ACTIVATION = 0.3  # Активация безубытка при достижении 30% от стоп-лосса
TRAILING_STEP = 0.1  # Шаг трейлинга (10% от расстояния прибыли)
PARTIAL_CLOSE_PCT = 0.5  # Процент частичного закрытия (50% от позиции)
USE_AUTO_CLOSE = True  # Использовать автоматическое закрытие позиций по определенным правилам
MAX_TRADE_DURATION = 12  # Максимальная длительность сделки в часах до автозакрытия

# Настройки адаптивного трейлинга для разных типов трендов
TRAILING_CONFIG = {
    "strong_trend": {
        "activation": 0.6,  # Более поздняя активация для сильного тренда
        "step": 0.08,       # Более осторожный шаг для сильного тренда
    },
    "weak_trend": {
        "activation": 0.4,  # Более ранняя активация для слабого тренда
        "step": 0.15,       # Более агрессивный шаг для слабого тренда
    },
    "sideways": {
        "activation": 0.3,  # Ранняя активация для бокового движения
        "step": 0.2,        # Агрессивный шаг для бокового движения
    }
}
